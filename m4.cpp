// echo server v4: epoll + 线程池 + 昵称系统 + 退格兼容
#include <iostream>
#include <cstring>
#include <string>
#include <queue>
...
void split_line_packet(Connection* conn, int fd, ThreadPool& pool)
{
    ...
    while ((pos = buf.find('\n')) != std::string::npos)
    {
        ...
        if (!conn->is_logined)
        {
            ...
            std::string welcome = "【系统】欢迎你，" + conn->nickname + "！当前在线人数：" + std::to_string(online_fds.size() + 1) + "\n";
            std::string join_tip = "【系统】用户 " + conn->nickname + " 加入了聊天室\n";
        ...
        pool.enqueue([nickname = conn->nickname, msg](){
            std::string broadcast = "[" + nickname + "]：" + msg + "\n";
            std::lock_guard<std::mutex> lock(main_task_mtx);
            for (int peer_fd : online_fds)
            {
                ...
                if (revents & EPOLLIN)
                {
                    ...
                    if(closed)
                    {
                        std::cout << "[主线程] fd=" << fd << " 客户端下线\n";
                        {
                            ...
                            if (conn->is_logined)
                            {
                                std::string leave_tip = "【系统】用户 " + conn->nickname + " 离开了聊天室\n";
                                online_fds.erase(fd);
