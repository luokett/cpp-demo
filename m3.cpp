...
{
    int fd;
    std::string recv_buf;   // 用户态接收缓冲区，存放不完整数据包（解决粘包）
    std::string send_buf;   // 用户态发送缓冲区，内核缓冲区满时缓存待发数据
...
// 可读事件（原有逻辑）
        if (revents & EPOLLIN)
        {
            ...
            if (bytes > 0)
            {
                conn->recv_buf.append(buffer, bytes);
                std::cout << "[主线程] 收到 " << bytes << " 字节 (fd=" << fd << ")\n";
                std::string request = conn->recv_buf;
                pool.enqueue([fd, request]() {
                    std::cout << "[工作线程] 处理 fd=" << fd << std::endl;
                    std::string resp = "echo reply:" + request;
                    {
                        std::lock_guard<std::mutex> lock(main_task_mtx);
                        main_task_queue.push({fd, resp});
                    }
                });
            }
            else
        ...
        if (revents & EPOLLOUT)
        {
            ...
            if(send_len > 0)
            {
                // 已经发出send_len字节，缓冲区截断前面已发送部分
                conn->send_buf = conn->send_buf.substr(send_len);
                std::cout << "[主线程] fd=" << fd << "发送" << send_len << "字节\n";
